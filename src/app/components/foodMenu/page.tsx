      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {foodItems.map((food, index) => (
          <div key={index} className="bg-white p-4 rounded-lg shadow-lg">
            {food.image && <img src={food.image} alt={food.food_name} className="w-full h-32 object-cover rounded-lg mb-4" />}
            ...
            <button onClick={() => editFoodItem(index)} className="text-black font-bold hover:text-purple-300 mr-2">Edit</button>
            <button onClick={() => deleteFoodItem(index)} className="text-red-500 font-bold hover:text-purple-300">Delete</button>
          </div>
        ))}
      </div>
    </div>
